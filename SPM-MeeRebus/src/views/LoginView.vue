      ...
      <form @submit.prevent="handleLogin">
        <div class="form-group">
          ...
          <input
            ...
            v-model="empId"
            class="form-control"
            placeholder="Please key in your employee ID"
            required
          />
        </div>
  ...
  methods: {
    async handleLogin() {
      ...
      if (isValid) {
        ...
        if (
          (employeeData && employeeData.position === 'Director') ||
          employeeData.position === 'MD'
        ) {
          eventBus.isDir = true;
        }
        ...
        async checkValidID() {
            try {
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/employee/${this.empId}`);
        ...
        if (
          ...
          response.data.position != 'Director'
        ) {
          // Only if HR, get all the arrangement data
          eventBus.isHR = true;
          localStorage.setItem('isHR', true);
        ...
        async getOwnArrangementData(staff_id) {
            try {
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/arrangement/emp/${staff_id}`);
        ...
        async getOwnUnderlingData(staff_id) {
            try {
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}//manager/underlings/${staff_id}`);

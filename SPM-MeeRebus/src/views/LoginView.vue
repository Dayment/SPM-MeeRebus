  ...
  methods: {
    async handleLogin() {
      ...
      if (isValid) {
        ...
        this.errorMessage =
          'Invalid Employee ID or not authorized to log in from this location. Please try again.';
      }
    },
    async checkValidID() {
      try {
        const response = await axios.get(
          `https://earnest-grace-production-04af.up.railway.app/employee/${this.empId}`,
        );

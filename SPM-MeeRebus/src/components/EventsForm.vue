<template>
  <form @submit.prevent="handleSubmit">
    ...
    >
      <option
        v-for="department in existingDepartment"
        :key="department.id"
        ...
        <tbody>
          <tr v-for="(description, date) in apiDateDict" :key="date">
            ...
            <td>
              ...
              <button @click="removeEvent(date)" class="btn btn-danger btn-sm">
                X
              </button>
            </td>
          </tr>
  ...
  methods: {
    ...
    async handleSubmit() {
      if (this.checkWhetherExistingDateOverlap(this.selectedDate)) {
        console.log('failure');
        window.alert(
          'This event date overlaps with an existing event. Please choose a different date.',
        );
      } else {
